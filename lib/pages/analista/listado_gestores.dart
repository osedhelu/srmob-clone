...
class _ListadoGestoresPageState extends State<ListadoGestoresPage> {
  Widget _buildGestor(
      BuildContext context, Usuario usuario, DocumentSnapshot gestor) {
    return Card(
        elevation: 3.0,
          ...
          if (uSnapshot.hasData && uSnapshot.data != null) {
            return Scaffold(
                ...
                body: StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (BuildContext gbContext,
                      AsyncSnapshot<QuerySnapshot> gSnapshot) {
                    if (gSnapshot.hasData && gSnapshot.data != null) {
                      return GridView.extent(
                        shrinkWrap: true,
                        maxCrossAxisExtent: 600,
                        childAspectRatio: 4,
                        children: gSnapshot.data.documents
                            .map((DocumentSnapshot documentSnapshot) =>
                                _buildGestor(gbContext, uSnapshot.data,
                                    documentSnapshot))
                            .toList(),
                      );
                    }
