...
class EditarMuestraOperadorPage extends StatefulWidget{    
    ...
    EditarMuestraOperadorPage({Key key, this.usuario, this.muestra}) : super(key: key);

    @override
    State<EditarMuestraOperadorPage> createState() => new _EditarMuestraOperadorState();
}
...
class _EditarMuestraOperadorState extends State<EditarMuestraOperadorPage>{
  ...
  void initState() {
    ...
    Firestore.instance.document("aparato/${this.widget.muestra['aparato']}")
      .get()
      .then(
        (DocumentSnapshot aparato){
          setState(() {
    ...
    return Card(      
      child: Stack(
        children: <Widget>[
          ...
          Positioned(
            right: 10.0, bottom: 3.0, left: 0, top: 18,
            child: AutoSizeText('${value??''}', style: Theme.of(context).textTheme.display1,textAlign: TextAlign.right, maxLines: 1, softWrap: true, ),
          )          
        ],
      )
    );
  ...
  List<Widget> _buildLegacyFields(List<Widget> beforeList, BuildContext context, List<Widget> afterList){
  ...
  Widget build(BuildContext context) {
    String grupo  = this.aparato != null ? this.aparato.data['grupos'][this.widget.muestra.data['color']] : "";
    String nombre = this.aparato != null ? this.aparato.data['nombre'] : '...'; 
    bool isFracciones = _isFracciones(this.widget.muestra.data['version'], grupo);
    return Scaffold(
      key: _scaffoldKey,
      appBar: AppBar(
        title: Text("$nombre", ),
              ...
              GridView.extent(               
                ...
                childAspectRatio: 4,
                  children: this._buildLegacyFields(
                    <Widget>[ 
                      _buildInfoCard(context, "# Muestra", this.widget.muestra.data['id']),
                    ],
                    context, 
                    <Widget>[                                                           
                      OutlineLabel(
                ...
                isFracciones
                ? RaisedButton(
                    child: Text( "Fracciones", style: Theme.of(context).textTheme.button.copyWith(color: Colors.white), ),  
                    onPressed: () async {
                      if( this._formKey.currentState.validate() ){
                        ...
                        if( this.widget.muestra["estado"] == 0 && _muestra["peso"] != null ){
                          _muestra["estado"] = 1;
                        }
                        await this.widget.muestra.reference.setData(
                          this._muestra,
                          merge: true
                        );
                        ...
                        Navigator.of(context).pop("Se ha actualizado la muestra y/o sus fracciones");
                      }
                    },
                  )                                  
                : RaisedButton(
                    child: Text( "Guardar", style: Theme.of(context).textTheme.button.copyWith(color: Colors.white), ),
                    onPressed: () async {
                      if( this._formKey.currentState.validate() ){
                        setState(() {
                          _saving = true;
                        ...
                        Navigator.of(context).pop("Se ha actualizado la muestra");
                      }
                    },                
                  ),              
              ],
