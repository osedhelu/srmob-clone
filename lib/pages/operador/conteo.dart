...
class ConteoPage extends StatelessWidget {
  ...
  Future<void> _showConfirmDialog(
      BuildContext context, DocumentSnapshot conteo) async {
  ...
  Widget _buildItem(BuildContext context, DocumentSnapshot conteo) {
    ...
    return FutureBuilder<DocumentSnapshot>(
        future: Firestore.instance
            .collection("aparato")
            .document(conteo.data['aparato'])
            .get(),
        builder: (BuildContext cbContext,
            AsyncSnapshot<DocumentSnapshot> cSnapshot) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: Firestore.instance
            ...
            .snapshots(),
        builder:
            (BuildContext cbContext, AsyncSnapshot<QuerySnapshot> cSnapshot) {
          if (cSnapshot.hasData && cSnapshot.data != null) {
            return GridView.extent(
              shrinkWrap: true,
              maxCrossAxisExtent: 600,
              childAspectRatio: 5,
              children: cSnapshot.data.documents
                  .map((DocumentSnapshot conteo) =>
                      _buildItem(cbContext, conteo))
                  .toList(),
            );
          }
