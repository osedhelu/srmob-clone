...
class ListadoMuestrasPage extends StatefulWidget {
  ...
  ListadoMuestrasPage({Key key, this.fechaSeleccionada, this.selectedIndex})
      : super(key: key);
...
class _ListadoMuestrasPageState extends State<ListadoMuestrasPage> {
  ...
  Widget _buildMuestra(
      BuildContext context, Usuario usuario, DocumentSnapshot muestra) {
        ...
        builder: (BuildContext sContext, AsyncSnapshot<Usuario> uSnapshot) {
          if (uSnapshot.hasData && uSnapshot.data != null) {
            ...
            if (this.widget.fechaSeleccionada == null) {
              ...
              query = query.where("fechaHora",
                  isLessThan:
                      this.widget.fechaSeleccionada.add(new Duration(days: 1)));
              query = query.orderBy("fechaHora", descending: true);
            }
            return Scaffold(
              key: _scaffoldKey,
                  ...
                  this._bottomSelectedIndex >= 0
                      ? StreamBuilder<QuerySnapshot>(
                          ...
                          builder: (BuildContext abContext,
                            ...
                            print(
                                "Filtrados por ${this._bottomSelectedIndex} ${aSnapshot.connectionState}");
                            if (aSnapshot.hasData && aSnapshot.data != null) {
                              List<Widget> childrens;
                              if (this.widget.fechaSeleccionada == null) {
                                childrens =
                                    groupByDate(aSnapshot.data.documents);
                              } else {
                                childrens = aSnapshot.data.documents
                                    .map((DocumentSnapshot analisis) =>
                                        _buildMuestra(abContext, uSnapshot.data,
                                            analisis))
                                    .toList();
                              }
