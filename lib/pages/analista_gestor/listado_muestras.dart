...
import 'package:araee/pages/analista_gestor/menu_analista.dart';
...
class ListadoMuestrasPage extends StatefulWidget{    
    ...
    ListadoMuestrasPage({Key key, this.fechaSeleccionada, this.selectedIndex}) : super(key: key);    
...
class _ListadoMuestrasPageState extends State<ListadoMuestrasPage>{
        ...
        .then(
          (_){
            if( this._sharedPreferences.containsKey("lmBottomSelectedIndex") ){
              setState(() {              
                this._bottomSelectedIndex = this._sharedPreferences.getInt("lmBottomSelectedIndex");
                print("lmBottomSelectedIndex :: get :: ${this._bottomSelectedIndex}");
              });
            }
            else{
  ...
  Widget _buildMuestra(BuildContext context, Usuario usuario, DocumentSnapshot muestra){
          ...
          return ListTile(
            ...
            trailing: Container(
              ...
              child: Stack(              
                ...
                children: <Widget>[
                  AutoSizeText(
                    _isFracciones( muestra['version'], aSnapshot.hasData && aSnapshot.data != null ? aSnapshot.data['grupos'][muestra['color']] : null )
                  ...
                  Positioned(
                    ...
                    child: Text(
                      ...
                      style: Theme.of(context).textTheme.caption.copyWith(fontSize: 10, color: Colors.white54),                    
                    ),
                  )                
                ],
              ),
            ...
            onTap: () async{
              //if( muestra.data['estado'] != 2 || muestra.data['usuario'] == usuario.id ){
              ...
              //  _scaffoldKey.currentState.showSnackBar(
              //    SnackBar(
              //      content: Text("La edición de esta muestra esta restringida a los analistas de nivel superior", 
      ...
      (String key, num qt){
        retValue.add(
          Card(
            elevation: 2,      
            child: ListTile(
              title: Text(key,style: Theme.of(context).textTheme.title,),
              trailing: Container(
                width: 48, height: 48,
                ...
                child: Center(
                  child: Text("$qt",style: Theme.of(context).textTheme.subtitle.copyWith(fontSize: 24, color: Colors.white),),
                ),
              ),
              onTap: () {                
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) => ListadoMuestrasPage(                      
                      fechaSeleccionada: DateFormat("yyyy/MM/dd HH:mm:ss").parse("$key 00:00:00"),
    ...
    return FutureBuilder<Usuario>(
      ...
      builder: (BuildContext sContext, AsyncSnapshot<Usuario> uSnapshot) {
        if( uSnapshot.hasData && uSnapshot.data != null ){
          Query query = Firestore.instance.collection("analisis")
              .where("gestor", isEqualTo: uSnapshot.data.gestor.documentID)
          ...
          if( this.widget.fechaSeleccionada == null ){
            ...
            print("Fecha: ${this.widget.fechaSeleccionada}");
            query = query.where("fechaHora", isGreaterThanOrEqualTo: this.widget.fechaSeleccionada);
            query = query.where("fechaHora", isLessThan: this.widget.fechaSeleccionada.add(new Duration(days: 1)));
            query = query.orderBy("fechaHora", descending: true);
          }   
          return Scaffold(            
            key: _scaffoldKey,
            appBar: AppBar(
              title: Text( this.widget.fechaSeleccionada != null ? "Muestras ${DateFormat("yyyy/MM/dd").format(this.widget.fechaSeleccionada)}" : "Muestras"),
              actions: <Widget>[
                IconButton(
                    ...
                    String message = await Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) => ListadoAparatosPage(
                          usuario: uSnapshot.data,
                          gestor: uSnapshot.data.gestor,
                        )
                      ),
                    );
                    if( message != null ){
              ...
              children: <Widget>[                
                ...
                ? StreamBuilder<QuerySnapshot>(
                    ...
                    builder: (BuildContext abContext, AsyncSnapshot<QuerySnapshot> aSnapshot) {
                      print("Filtrados por ${this._bottomSelectedIndex} ${aSnapshot.connectionState}");
                      if( aSnapshot.hasData && aSnapshot.data != null ){   
                        ...
                        if( this.widget.fechaSeleccionada == null ){
                          ...
                          childrens = aSnapshot.data.documents.map((DocumentSnapshot analisis) => _buildMuestra(abContext, uSnapshot.data, analisis)).toList();
                        }                  
